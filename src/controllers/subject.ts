...
import { insertSubject,getSubject,getSubjects,updateSubject,deleteSubject, matriculateSubject, getSubjectsFiltered, getUsersFiltered} from "../services/subject";
...
const get_Subjects_Filtered =async ({params}:Request, res:Response) => {
    try{
        const {idUser} = params;
        const response = await getSubjectsFiltered(idUser);
        const data = response ? response: "USER_NOT_FOUND";
        res.send(data);
    }catch(e){
...
const get_Users_Filtered =async ({params}:Request, res:Response) => {
    try{
        const {idSubject} = params;
        const response = await getUsersFiltered(idSubject);
        const data = response ? response:"SUBJECT_NOT_FOUND";
        res.send(data);
    }catch(e){
...
export{get_Subject,get_Subjects,post_Subject,update_Subject,delete_Subject,get_Users_Filtered, matriculate_Subject, get_Subjects_Filtered};